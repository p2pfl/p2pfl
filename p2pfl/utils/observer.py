...
class Events():
    ...
    Attributes:
        SEND_BEAT_EVENT: Used to notify that beats must be sent.
        END_CONNECTION_EVENT: Used to notify that a connection has been closed. (arg: NodeConnection)
        AGREGATION_FINISHED_EVENT: Used to notify that the agregation was done. (arg: model or None)
        CONN_TO_EVENT: Used to notify when a node must connect to other. (arg: (host,port))
        START_LEARNING_EVENT: Used to notify when the learning process starts. (arg: (rounds,epochs))
        STOP_LEARNING_EVENT: Used to notify when the learning process stops.
        PARAMS_RECEIVED_EVENT: Used to notify when the parameters are received. (arg: params (encoded))
        METRICS_RECEIVED_EVENT: Used to notify when the metrics are received. (arg: (node, round, loss, metric))
        TRAIN_SET_VOTE_RECEIVED_EVENT: Used to notify when a vote is received. (arg: (node,votes))
        NODE_CONNECTED_EVENT: Used to notify when a node is connected. (arg: (n, force))
        PROCESSED_MESSAGES_EVENT: Used to notify when a node process messages. (arg: (node, messages))
        GOSSIP_BROADCAST_EVENT: Used to notify when a node must send gossiped messages. (arg: (msg,nodes))
        BEAT_RECEIVED_EVENT: Used to notify when a node receive a beat. (arg: node)
    """
