...
from p2pfl.communication.protocols.edge.server import EdgeServerConnection
...
from p2pfl.stages.workflows import ProxyWorkflow
...
class ProxyNode:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize a node."""
        # P2P Communication protol
        self._communication_protocol = protocol(address)
        self.addr = self._communication_protocol.get_address()

        # Edge Communication protol
        self._edge_communication_protocol = EdgeServerConnection(proxy_address) # TMP - MEJORAR ADDRESS

        # Learning
        self.learner = learner(model, None, self.addr) # Vestigial: LEARNER NOT REQUIRED IN PROXY NODE
        ...
        # Workflow
        self.learning_workflow = ProxyWorkflow()
    ...
    def start(self, wait: bool = False) -> None:
        ...
        # Edge Communication Protocol
        self._edge_communication_protocol.start()
        # Wait
        if wait:
    ...
    def __start_learning(self, rounds: int, epochs: int) -> None:
        try:
            self.learning_workflow.run(
                ...
                edge_communication_protocol=self._edge_communication_protocol
            )
