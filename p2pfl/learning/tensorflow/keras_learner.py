...
class KerasLearner(NodeLearner):
    def __init__(
        self, model: tf.keras.Model, data: Tuple[tf.data.Dataset, tf.data.Dataset], self_addr: str, epochs: int
    ):
        self.model = model
        self.data = data
    ...
    def decode_parameters(self, data: bytes) -> LearnerStateDTO:
        try:
            ...
            raise DecodingParamsError(f"Error in decoding parameters with Keras: {e}")
