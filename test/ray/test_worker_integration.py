...
async def test_multiple_nodes_can_fit_concurrently(mock_pool_and_worker):
    ...
    # Mock fit.remote to return models directly (async methods use await, not ray.get)
    mock_models = [MagicMock(spec=P2PFLModel) for _ in range(3)]
    call_count = 0

    async def fit_side_effect(node_key):
        nonlocal call_count
        result = mock_models[call_count]
        call_count += 1
        return result
...
def test_node_address_change_rekeys_correctly(mock_pool_and_worker):
    ...
    mock_pool_and_worker["worker"].rekey_and_set_address.remote.reset_mock()

    # Change node1's address
    node1.set_address("node-0-new")

    # Verify rekey_and_set_address.remote was called once with correct old/new keys
    mock_pool_and_worker["worker"].rekey_and_set_address.remote.assert_called_once_with("node-0", "node-0-new")
